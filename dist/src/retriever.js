    ...
    async hybridRetrieval(query, limit, scopeFilter, category, trace, source, diagnostics) {
        ...
        try {
            ...
            const [vectorResults, bm25Results] = await Promise.all([
                this.runVectorSearch(queryVector, candidatePoolSize, scopeFilter, category).catch((error) => {
                    throw attachFailureStage(error, "hybrid.vectorSearch");
                }),
                this.runBM25Search(bm25Query, candidatePoolSize, scopeFilter, category).catch((error) => {

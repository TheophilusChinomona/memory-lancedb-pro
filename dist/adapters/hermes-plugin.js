...
import { buildSessionKey, lookupSession, registerSession, touchSession, markDormant, } from "./session-recovery.js";
// ============================================================================
// Default Config
// ============================================================================
...
export class HermesMemoryEngine {
     ...
     */
    resolveSession(ctx) {
        const sessionKey = buildSessionKey(ctx.platform, ctx.channelId || "default", ctx.threadId);
     ...
     */
    sessionDormant(platform, channelId, threadId) {
     ...
     */
    async autoRecall(messageText, scope) {
        if (!this.initialized || !this.config.autoRecall)
            return null;
        ...
        try {
            ...
            if (!results || results.length === 0)
                return null;
            const memoryLines = results.map((r) => `- ${r.entry.text}`);
     ...
     */
    async autoCapture(messages, scope) {
...
// Factory
// ============================================================================
export function createHermesMemoryEngine(config, logger) {
